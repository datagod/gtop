...
namespace Config {
	...
	const vector<array<string, 2>> descriptions = {
		{"color_theme", 		"#* Name of a gtop++/btop++/bpytop/bashtop formatted \".theme\" file, \"Default\" and \"TTY\" for builtin themes.\n"
								"#* Themes should be placed in \"../share/gtop/themes\" relative to binary or \"$HOME/.config/gtop/themes\""},
		...
		{"log_level", 			"#* Set loglevel for \"~/.local/state/gtop.log\" levels are: \"ERROR\" \"WARNING\" \"INFO\" \"DEBUG\".\n"
		...
		{"workload_panel_max_height", "#* Minimum terminal rows for the GPU workloads panel (8-80); in CPU+GPU layout the panel grows to fill remaining space."},
	...
	auto get_log_file() -> std::optional<fs::path> {
		return get_xdg_state_dir().transform([](auto&& state_home) -> auto { return state_home / "gtop.log"; });
	}

	auto current_config() -> std::string {
		auto buffer = std::string {};
		fmt::format_to(std::back_inserter(buffer), "#? Config file for gtop++ v.{}\n", Global::Version);
